...
use crate::{holdable, level, Position, GameObject, EntityType};

pub struct BlockObject { 
    pub target: Option::<crate::Direction>,
}

pub fn update_block(
    mut blocks: Query<(Entity, &mut BlockObject, &mut Position, &mut Transform), Without<holdable::BeingHeld>>, 
    mut level: ResMut<level::Level>,
    time: Res<Time>, 
) {
    for (entity, mut block_object, mut position, mut transform) in blocks.iter_mut() {
        if !block_object.target.is_some() { 
        ...
        let target_translation = match block_object.target.unwrap() {
