...
use crate::editor::{GameEntityType, GameEntity, add_entity};
use crate::editor::properties::{Properties, block::BlockProperties, not_snake::NotSnakeProperties};
                    ...
                    if let Some((entity, intersection)) = picking_camera.intersect_top() {
                        match *entity_action {
                            ...
                            EntityAction::Add => {
                                ...
                                items_with_selections.iter_mut()
...
pub fn detect_entity_selections(
    ...
    if *entity_action != EntityAction::Select { return; }
...
pub fn store_selected_values(
    ...
    if selected_entities.is_empty() || last_stored_entity.map_or(false, |e| selected_entities.contains(&e)) {
