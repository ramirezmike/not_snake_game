use bevy::prelude::*;
use crate::{AppState};
use crate::editor::{editor_camera, help_text::HelpTextBoxEvent, EditorTrashMarker, play, GameEntity};
...
fn create_editor_ui( 
    ...
    create_entity_button(&mut commands, &asset_server, 1, "Block", EntityButton::Block);
    create_entity_button(&mut commands, &asset_server, 2, "Not Snake", EntityButton::NotSnakeSpawn);
    create_entity_button(&mut commands, &asset_server, 3, "Snake", EntityButton::SnakeSpawn);
}
...
fn handle_button_interaction(
    interface_button_query: Query<(&Interaction, &InterfaceButtonComponent), (Changed<Interaction>, With<Button>)>,
    entity_button_query: Query<(&Interaction, &EntityButtonComponent), (Changed<Interaction>, With<Button>)>,
    ...
    for (interaction, button) in interface_button_query.iter() {
        match *interaction {
            Interaction::Clicked => {
                if !interface.selected_button.is_none() && interface.selected_button.as_ref().unwrap() == &button.interface_button {
                    // Deselect button
                    ...
                    match button.interface_button {
                        ...
                        InterfaceButton::Play => 
                            interface_event_writer.send(InterfaceActionEvent { action: InterfaceAction::Play }),
                        InterfaceButton::Select => (),
                        InterfaceButton::Add => 
                            interface_event_writer.send(InterfaceActionEvent { action: InterfaceAction::Add }),
                ...
                interface.selected_button = match interface.current_state() {
                                                ...
                                                _ => {
                                                    help_text_box_event_writer.send(
                                                        HelpTextBoxEvent { text: Some("Press 'C' to exit Camera mode".to_string()) });
