use bevy::prelude::*;
use crate::editor::{GameEntity, GameEntityType, editor_camera, cleanup_editor};
use crate::{AppState, sounds, environment, level, snake, dust, dude, path_find, food, game_controller, holdable, moveable};
    ...
    fn build(&self, app: &mut App) {
        app.add_system_set(
            SystemSet::on_enter(AppState::EditorPlay)
                       ...
                       .with_system(environment::load_level.label("loading_level").after("load_level_from_editor"))
                       .with_system(environment::load_level_into_path_finder.after("loading_level"))
        )
        .insert_resource(CurrentEditorLevel { 
            level_info: None,
        })
        .add_system_set(
